...
class Solution {
    // edge from node[i], pair<toNode, cost>
    vector<vector<pair<int, int>>> convertNodeEdges(int n, const vector<vector<int>>& edges) {
        vector<vector<pair<int, int>>> nodeEdges(n);
        for (int i = 0; i < edges.size(); i++) {
            nodeEdges[edges[i][0]].push_back(pair<int, int>{edges[i][1], edges[i][2]});
            nodeEdges[edges[i][1]].push_back(pair<int, int>{edges[i][0], edges[i][2] * 2});
        }
        return move(nodeEdges);
    }
public:
    int minCost(int n, vector<vector<int>>& edges) {
        ...
        if (costToNode.back() == INT_MAX) {
            return -1;
        }
        return costToNode.back();
    }
};

...
public:
    int earliestFinishTime(vector<int>& landStartTime, vector<int>& landDuration, vector<int>& waterStartTime, vector<int>& waterDuration) {
        int out = INT_MAX;
        int landEndTime = INT_MAX;
        for (int i = 0; i < landStartTime.size(); i++) {
            landEndTime = min(landEndTime, landStartTime[i] + landDuration[i]);
        }
        for (int i = 0; i < waterStartTime.size(); i++) {
            if (waterStartTime[i] >= landEndTime) {
                out = min(out, waterStartTime[i] + waterDuration[i]);
            } else {
                out = min(out, landEndTime + waterDuration[i]);
            }
        }
        int waterEndTime = INT_MAX;
        for (int i = 0; i < waterStartTime.size(); i++) {
            waterEndTime = min(waterEndTime, waterStartTime[i] + waterDuration[i]);
        }
        for (int i = 0; i < landStartTime.size(); i++) {
            if (landStartTime[i] >= waterEndTime) {
                out = min(out, landStartTime[i] + landDuration[i]);
            } else {
                out = min(out, waterEndTime + landDuration[i]);
            }
        }
        return out;
    }
